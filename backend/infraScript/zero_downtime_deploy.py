...
class ServiceManager:
    # 초기화 함수
    def __init__(self, socat_port: int = 7000, sleep_duration: int = 3) -> None:
        self.socat_port: int = socat_port
        self.sleep_duration: int = sleep_duration
        self.services: Dict[str, int] = {
            'wine_log_backend_1': 7001,
            'wine_log_backend_2': 7002
        }
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'wine_log_backend_2', self.services['wine_log_backend_2']
        else:
    ...
    def _cleanup_dangling_images(self) -> None:
        ...
        cmd_list = "docker images | grep '.*/wine_log_backend/wine_log_backend' | grep '<none>'"
        ...
        # 이미지 정리
        cmd_remove = "docker images | grep '.*/wine_log_backend/wine_log_backend' | grep '<none>' | awk '{print $3}' | xargs -r docker rmi"
    ...
    def _is_service_up(self, port: int) -> bool:
        url = f"http://127.0.0.1:{port}/actuator/health"  # FastAPI 헬스체크 엔드포인트로 조정
        try:
